...
sf::Sprite *Renderer::get_column(sf::Texture *texture, const Vertex &vertex,
  ...
  if (texX + texWidth > static_cast<int>(texture->getSize().x)) {
    texWidth = static_cast<int>(texture->getSize().x) - texX;
  }
  ...
  sprite->setTextureRect(sf::IntRect(texX, 0, texWidth, texture->getSize().y));
  return sprite;
}
