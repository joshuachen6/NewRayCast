...
private:
  ...
  sf::Sprite *get_column(sf::Texture *texture, const Vertex &vertex, sf::Vector2f &collision, int cols);
...
public:
  ...
  void update(World &world, sf::Vector3f &camera, double fov, double rays, double dt);
  ...
  void drawText(sf::Vector2f position, std::string font, std::string text, int size, sf::Color color);
  void drawTextWrapped(sf::Vector2f position, std::string font, std::string text, int size, sf::Color color, int width);
